...
def evaluating(experiment_name, model_name, batch_size, gpu_id, checkpoint_path, dataset_name, dataset_path, dataset_csv):
    ...
    if dataset_name == 'casia1':
        eval_data = Fusion_Dataset(dataset_path, dataset_csv, 
            transform = evaluation_augmentations,split=['train','val'], origin=['casia1'], dct_flag = flag_dct, sb_flag = flag_sb)
    if dataset_name == 'ifs_tc':
        eval_data = IFS_TC(dataset_path, '../dataset/csv/test.csv', transform=evaluation_augmentations, split=['test'], dct_flag=flag_dct, sb_flag=flag_sb)
    if dataset_name == 'columbia':
        eval_data = Columbia_Dataset(dataset_path, '/home/siopi/drive2/Columbia/columbia_dataset.csv', transform = evaluation_augmentations, split=None, dct_flag=flag_dct, sb_flag=flag_sb)
