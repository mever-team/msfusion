...
def training(experiment_name, model_name, batch_size, gpu_id, learning_rate, epochs, checkpoint_path, dataset_name, dataset_path, dataset_csv):
    ...
    if dataset_name == 'synthetic':
        train_data = Synthetic_Dataset(dataset_path, dataset_csv, transform=train_augmentations, dct=flag_dct, sb=flag_sb)
        val_data = Synthetic_Dataset(dataset_path, dataset_csv, transform=val_augmentations, split=['val'], dct=flag_dct, sb=flag_sb)
    if dataset_name == 'casia2':
        train_data = Fusion_Dataset(dataset_path, dataset_csv, 
                            transform = train_augmentations, origin=['casia2'], dct_flag=flag_dct, sb_flag=flag_sb)
        val_data = Fusion_Dataset(dataset_path, dataset_csv, 
                                  transform=val_augmentations, split=['val'], origin=['casia2'], dct_flag=flag_dct, sb_flag=flag_sb)
    if dataset_name == 'ifs_tc':
        train_data = IFS_TC(dataset_path,'../dataset/csv/test.csv', transform=train_augmentations,
                   image_size=256,to_tensor=True, verbose=False, dct_flag=flag_dct, sb_flag=flag_sb)
        val_data = IFS_TC(dataset_path, '../dataset/csv/test.csv', 
                                  transform=val_augmentations, split=['val'], dct_flag=flag_dct, sb_flag=flag_sb)
